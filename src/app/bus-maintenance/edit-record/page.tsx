...
const EditPage = () => {
    ...
    const fetchMaintenanceData = async () => {
      try {
        if (maintenanceId) {
          ...
          setMaintenanceDate(new Date(maintenanceData.maintenance_date));
          setMaintenanceAddress(maintenanceData.maintenance_address);
          setMaintenanceType(maintenanceData.maintenance_type);
          setAttendingMechanic(maintenanceData.attending_mechanic);
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    const data = {
      ...
      maintenance_status: "active", // Keep the maintenance status active
    };

    try {
      await updateMaintenanceScheduling(maintenanceId, data); // PUT request to update maintenance
